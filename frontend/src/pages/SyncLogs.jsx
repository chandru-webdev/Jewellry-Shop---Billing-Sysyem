    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-3 mb-5">
        <div className="bg-white rounded-xl border border-gray-200/80 shadow-sm p-4">
          <p className="text-[11px] uppercase tracking-wider text-gray-500 font-medium">Total Synced</p>
      ...
      <div className="flex flex-wrap items-center gap-3 mb-4">
        ...
        <select value={filterEntity} onChange={(e) => setFilterEntity(e.target.value)} className="text-sm bg-white border border-gray-200 rounded-lg px-3 py-2 text-gray-700 focus:outline-none focus:ring-2 focus:ring-royal-500">
      ...
      <Card className="p-0 overflow-hidden">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 text-left">
                ...
                <th className="px-4 py-3 font-medium text-gray-600">Shopify ID</th>
                <th className="px-4 py-3 font-medium text-gray-600">Entity Name</th>
            ...
            <tbody>
              {filtered.map((log, i) => {
                ...
                return (
                  <tr key={log.id} className={`border-t border-gray-100 ${i % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'} hover:bg-royal-50/30 transition-colors`}>
                    ...
                    <td className="px-4 py-2.5 font-medium text-royal-950 text-xs max-w-52 truncate">{log.entityName}</td>
                    <td className="px-4 py-2.5 text-xs text-gray-600 whitespace-nowrap">{log.direction}</td>
                    <td className="px-4 py-2.5 text-xs text-gray-600">{log.action}</td>
                    <td className="px-4 py-2.5">
                      <Badge tone={statusColor[log.status]}>
                        <StatusIcon size={11} className="mr-1" /> {log.status}
                      </Badge>
                    </td>
                    <td className="px-4 py-2.5 text-xs text-gray-500 whitespace-nowrap">{log.time}</td>
                    <td className="px-4 py-2.5 font-mono text-xs text-gray-600">{log.duration}</td>
              ...
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={10} className="px-4 py-8 text-center text-sm text-gray-400">No sync logs match your filters</td>
                </tr>
              )}
            </tbody>
          </table>
        ...
        <div className="flex items-center justify-between px-4 py-3 border-t border-gray-100 bg-gray-50/50">
          <p className="text-xs text-gray-500">Showing 1-{filtered.length} of {filtered.length} entries</p>
          <div className="flex gap-1">
            <button disabled className="px-3 py-1 text-xs rounded-md bg-white border border-gray-200 text-gray-400 cursor-not-allowed">Previous</button>
            <button className="px-3 py-1 text-xs rounded-md bg-royal-700 text-white cursor-pointer">1</button>
            <button disabled className="px-3 py-1 text-xs rounded-md bg-white border border-gray-200 text-gray-400 cursor-not-allowed">Next</button>
          </div>
        </div>
      </Card>
    </div>
