  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap items-center gap-3 mb-4">
        ...
        <select value={roleFilter} onChange={(e) => setRoleFilter(e.target.value)} className="text-sm border border-gray-200 rounded-lg px-3 py-2 bg-white focus:outline-none focus:ring-2 focus:ring-royal-500">
          <option value="">All Roles</option>
          {(roles || []).map((r) => <option key={r.id} value={r.name}>{r.name.replace(/_/g, ' ')}</option>)}
        </select>
        <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="text-sm border border-gray-200 rounded-lg px-3 py-2 bg-white focus:outline-none focus:ring-2 focus:ring-royal-500">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {isLoading ? (
                <tr><td colSpan="6" className="px-4 py-12 text-center text-gray-400"><Loader2 size={20} className="animate-spin mx-auto mb-2" />Loading users...</td></tr>
              ) : filtered.length === 0 ? (
                ...
                filtered.map((u) => (
                  <tr key={u.id} className="hover:bg-royal-50/30 transition-colors">
                    ...
                    <td className="px-4 py-3 text-xs text-gray-500">{u.lastLogin ? formatDateTime(u.lastLogin) : <span className="text-gray-300">Never</span>}</td>
          ...
          {!selectedUser && (
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                Password <span className="text-gray-400 font-normal">(leave blank for auto-generated temporary password)</span>
              </label>
      ...
      >
        {!resetResult ? (
          ...
          <div className="space-y-3">
            <p className="text-sm text-gray-600">New temporary password for <strong>{resetResult.email}</strong>:</p>
            ...
            <p className="text-xs text-gray-400">Share this password securely with the user. They will be asked to change it on first login.</p>
          </div>
        )}
      </Modal>
    </div>
