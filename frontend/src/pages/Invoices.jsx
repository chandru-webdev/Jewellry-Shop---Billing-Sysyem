    ...
    <div className="text-sm space-y-5">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-royal-50/60 rounded-lg p-3">
          <p className="text-[10px] uppercase tracking-wider text-gray-500 mb-1 font-semibold">Billed To</p>
          <p className="font-semibold text-royal-950">{invoice.customer?.name || 'Walk-in Customer'}</p>
          <p className="text-gray-600 text-xs">{invoice.customer?.phone}</p>
          {invoice.customer?.email && <p className="text-gray-600 text-xs">{invoice.customer.email}</p>}
        </div>
        <div className="bg-royal-50/60 rounded-lg p-3">
          <p className="text-[10px] uppercase tracking-wider text-gray-500 mb-1 font-semibold">Payment Info</p>
          <p className="font-semibold text-royal-950">{invoice.paymentMethod || '—'}</p>
          {invoice.salesperson?.name && <p className="text-gray-600 text-xs">Salesperson: {invoice.salesperson.name}</p>}
        </div>
      </div>
        ...
        <tbody className="divide-y divide-gray-100">
          {invoice.items?.map((it) => (
            <tr key={it.id}>
              ...
              <td className="px-3 py-2 text-center text-gray-600">{formatWeight(it.weight)}</td>
              <td className="px-3 py-2 text-center text-gray-600">{formatINR(it.silverRate)}</td>
      ...
      <div className="space-y-1.5 border-t border-gray-100 pt-3 text-sm">
        <div className="flex justify-between text-gray-500"><span>Total Weight</span><span>{formatWeight(invoice.totalWeight)}</span></div>
        <div className="flex justify-between text-gray-500"><span>Silver Value</span><span>{formatINR(invoice.subtotal)}</span></div>
        <div className="flex justify-between text-gray-500"><span>GST (3%)</span><span>{formatINR(invoice.gstTotal)}</span></div>
        {Number(invoice.discount) > 0 && (
          <div className="flex justify-between text-gray-500"><span>Discount</span><span className="text-red-500">- {formatINR(invoice.discount)}</span></div>
        )}
      ...
      <Card className="p-0 overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {isLoading && <tr><td colSpan={8} className="px-4 py-12 text-center text-gray-400">Loading invoices...</td></tr>}
              {!isLoading && filtered.length === 0 && <tr><td colSpan={8} className="px-4 py-12 text-center text-gray-400">No invoices found.</td></tr>}
              {filtered.map((inv) => (
                <tr key={inv.id} className="hover:bg-royal-50/30 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    {inv.orderNumber ? <Badge tone="blue">#{inv.orderNumber}</Badge> : <span className="text-gray-400 text-xs">—</span>}
                  </td>
                  <td className="px-4 py-3 font-medium text-royal-950">{inv.customer?.name || 'Walk-in'}</td>
                  ...
                  <td className="px-4 py-3 text-gray-600 text-xs">{formatDate(inv.date)}</td>
                  <td className="px-4 py-3">
