    ...
    <div className="text-sm space-y-5">
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-xs text-gray-400">Silver Jewellery · Purchase Invoice</p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-royal-50/60 rounded-lg p-3">
          <p className="text-[10px] uppercase tracking-wider text-gray-500 mb-1 font-semibold">Billed From</p>
          ...
          {invoice.customer?.address && <p className="text-gray-600 text-xs">{invoice.customer.address}</p>}
        </div>
        <div className="bg-royal-50/60 rounded-lg p-3">
          ...
          {invoice.salesperson?.name && <p className="text-gray-600 text-xs">Received by: {invoice.salesperson.name}</p>}
        </div>
      </div>
          ...
          <tbody className="divide-y divide-gray-100">
            ...
            {!isLoading && filtered.map((inv) => (
              <tr key={inv.id} className="hover:bg-royal-50/30 transition-colors">
                ...
                <td className="px-4 py-3">
                  <span className="font-medium text-royal-950">{inv.customer?.name || '—'}</span>
                ...
                <td className="px-4 py-3 text-center text-gray-600">{inv.paymentMethod || '—'}</td>
