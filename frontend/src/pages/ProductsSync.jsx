...
import { RefreshCw, Upload, Download, Search, CheckCircle2, Loader2 } from 'lucide-react'
...
export default function ProductsSync() {
  ...
  const handlePull = () => {
    setLoading('pull')
    setTimeout(() => {
      setProducts(products.map((p) => ({ ...p, lastSync: now(), erpMapped: true })))
      setLoading(null); showToast('Pulled all products from Shopify')
    }, 1500)
  }
  ...
  const handlePush = () => {
    setLoading('push')
    setTimeout(() => {
      setProducts(products.map((p) => ({ ...p, lastSync: now() })))
      setLoading(null); showToast('Pushed all products to Shopify')
    }, 1500)
  }
  ...
  const handleSync = (shopifyId) => {
    setLoading(`sync-${shopifyId}`)
    setTimeout(() => {
      setProducts(products.map((p) => p.shopifyId === shopifyId
        ? { ...p, lastSync: now(), erpMapped: true }
        : p))
      setLoading(null); showToast('Product synced')
    }, 1200)
  }
  ...
  const filtered = products.filter((p) => {
    ...
    return p.title.toLowerCase().includes(q) || p.sku.toLowerCase().includes(q) || p.shopifyId.toLowerCase().includes(q)
  })
    ...
    <div>
      <PageHeader title="Products Sync" subtitle="Manage Shopify product sync with ERP inventory" actions={
        <div className="flex gap-2">
          ...
          <Button variant="outline" size="sm" onClick={handlePush} disabled={!!loading}>
            {loading === 'push' ? <><Loader2 size={14} className="animate-spin" /> Pushing...</> : <><Upload size={14} /> Push to Shopify</>}
          </Button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-3 mb-5">
        <div className="bg-white dark:bg-[#1a1025] rounded-xl border border-gray-200 dark:border-white/[0.08]/80 shadow-sm p-4">
          <p className="text-[11px] uppercase tracking-wider text-gray-500 dark:text-gray-400 font-medium">Total Products</p>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 dark:bg-white/5 text-left">
                ...
                <th className="px-4 py-3 font-medium text-gray-600 dark:text-gray-400">Last Sync</th>
            ...
            <tbody>
              {filtered.map((p, i) => (
                <tr key={p.shopifyId} className={`border-t border-gray-100 dark:border-white/[0.05] ${i % 2 === 0 ? 'bg-white dark:bg-[#1a1025]' : 'bg-gray-50/50'}`}>
                  ...
                  <td className="px-4 py-2.5 text-center text-gray-600 dark:text-gray-400">{p.weight}</td>
                  <td className="px-4 py-2.5 text-center font-mono text-gray-600 dark:text-gray-400">{p.stock}</td>
                  ...
                  <td className="px-4 py-2.5 text-xs text-gray-500 dark:text-gray-400">{p.lastSync || 'Never'}</td>
                  <td className="px-4 py-2.5 text-right">
                    <Button variant="ghost" size="sm" onClick={() => handleSync(p.shopifyId)} disabled={!!loading}>
                      {loading === `sync-${p.shopifyId}` ? <Loader2 size={12} className="animate-spin" /> : <><RefreshCw size={12} /> Sync</>}
                    </Button>
                  </td>
                </tr>
              ))}
              {filtered.length === 0 && (
                <tr><td colSpan="9" className="px-4 py-8 text-center text-sm text-gray-400">No products found</td></tr>
              )}
            </tbody>
          </table>
        </div>
