  ...
  return (
    <div>
      ...
      <div className="mb-4 flex items-center gap-2">
        ...
        {isManager && (
          <span className="text-sm text-gray-500">You can submit rate requests for admin approval</span>
        )}
        {isSuperAdmin && (
          <span className="text-sm text-gray-500">You can directly update rates or approve requests</span>
        )}
      </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 mb-6">
            ...
            <Card className="lg:col-span-1">
              <div className="text-center py-4">
                ...
                {ratesLoading ? (
                  <div className="h-10 flex items-center justify-center"><Loader2 className="animate-spin text-gray-400" size={20} /></div>
                ) : (
            ...
            <Card title="Rate History" icon={TrendingUp}>
              <div className="h-52">
                {historyLoading ? (
                  ...
                  <div className="h-full flex items-center justify-center text-sm text-gray-400">No history yet</div>
                )}
              </div>
            </Card>
          </div>
            ...
            {previewData && (
              <div className="space-y-4">
                ...
                <div className="bg-gray-50 rounded-lg p-3 grid grid-cols-3 gap-3 text-center">
                  <div>
                    <p className="text-[10px] uppercase tracking-wider text-gray-500 font-semibold">Old Rate</p>
                    <p className="text-lg font-bold text-gray-700">₹{parseFloat(previewData.oldRate).toFixed(2)}</p>
                  </div>
                  <div>
                    <p className="text-[10px] uppercase tracking-wider text-gray-500 font-semibold">New Rate</p>
                    <p className="text-lg font-bold text-royal-900">₹{parseFloat(previewData.newRate).toFixed(2)}</p>
                  </div>
                  <div>
                    <p className="text-[10px] uppercase tracking-wider text-gray-500 font-semibold">Products</p>
                    <p className="text-lg font-bold text-royal-900">{previewData.affectedCount}</p>
                  </div>
                </div>
...
function RateRequestsTab() {
  ...
  return (
    <Card title="Rate Change Requests" icon={Clock}>
      ...
      {requestsLoading ? (
        <div className="py-12 flex items-center justify-center"><Loader2 className="animate-spin text-gray-400" size={24} /></div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200">
                <th className="text-left py-2.5 text-[11px] font-semibold uppercase tracking-wider text-gray-500">Requested By</th>
                ...
                <th className="text-left py-2.5 text-[11px] font-semibold uppercase tracking-wider text-gray-500">Products</th>
                <th className="text-left py-2.5 text-[11px] font-semibold uppercase tracking-wider text-gray-500">Status</th>
                <th className="text-left py-2.5 text-[11px] font-semibold uppercase tracking-wider text-gray-500">Requested</th>
                <th className="text-left py-2.5 text-[11px] font-semibold uppercase tracking-wider text-gray-500">Reviewed</th>
                <th className="text-center py-2.5 text-[11px] font-semibold uppercase tracking-wider text-gray-500">Action</th>
              </tr>
            </thead>
            <tbody>
                ...
                return (
                  <tr key={req.id} className="border-b border-gray-50 last:border-0">
                    <td className="py-2.5 font-medium text-royal-950">
                      {req.requestedBy?.name} <span className="text-gray-500 font-normal">({req.requestedBy?.email})</span>
                    </td>
                    <td className="py-2.5 text-right text-gray-600">₹{parseFloat(req.oldRate).toFixed(2)}</td>
                    ...
                    <td className="py-2.5 text-sm text-gray-500">{formatDateTime(req.createdAt)}</td>
