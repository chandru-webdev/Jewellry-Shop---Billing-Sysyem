    ...
    <div>
      ...
      <div className="mb-5">
        <div className="flex gap-2 border-b border-gray-200">
          <button onClick={() => setView('accounts')} className={`px-4 py-2 text-sm font-medium border-b-2 transition-colors ${view === 'accounts' ? 'border-royal-600 text-royal-700' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>Chart of Accounts</button>
          <button onClick={() => setView('trial')} className={`px-4 py-2 text-sm font-medium border-b-2 transition-colors ${view === 'trial' ? 'border-royal-600 text-royal-700' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>Trial Balance</button>
        </div>
      </div>
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-5">
            <div className="bg-white rounded-xl border border-gray-200/80 shadow-sm p-4">
              <p className="text-[11px] uppercase tracking-wider text-gray-500 font-medium">Total Assets</p>
              <p className="text-xl font-bold text-blue-600 mt-0.5">{formatINR(totalAssets)}</p>
            </div>
            <div className="bg-white rounded-xl border border-gray-200/80 shadow-sm p-4">
              <p className="text-[11px] uppercase tracking-wider text-gray-500 font-medium">Total Liabilities</p>
              <p className="text-xl font-bold text-red-600 mt-0.5">{formatINR(totalLiabilities)}</p>
            </div>
            <div className="bg-white rounded-xl border border-gray-200/80 shadow-sm p-4">
              <p className="text-[11px] uppercase tracking-wider text-gray-500 font-medium">Equity</p>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-royal-50/80 border-b border-gray-200">
                    <th className="px-4 py-3 text-left text-[11px] font-semibold uppercase tracking-wider text-gray-600">Code</th>
                    ...
                    <th className="px-4 py-3 text-center text-[11px] font-semibold uppercase tracking-wider text-gray-600">Sub-Type</th>
                    <th className="px-4 py-3 text-right text-[11px] font-semibold uppercase tracking-wider text-gray-600">Opening</th>
                    <th className="px-4 py-3 text-right text-[11px] font-semibold uppercase tracking-wider text-gray-600">Current Balance</th>
                ...
                <tbody className="divide-y divide-gray-100">
                  {filtered.map((a) => (
                    <tr key={a.id} className="hover:bg-royal-50/30 transition-colors">
                      ...
                      <td className="px-4 py-3 text-center text-sm text-gray-600">{a.subType}</td>
                      <td className="px-4 py-3 text-right font-mono text-sm text-gray-500">{formatINR(a.openingBalance)}</td>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-royal-50/80 border-b border-gray-200">
                  <th className="px-4 py-3 text-left text-[11px] font-semibold uppercase tracking-wider text-gray-600">Code</th>
                  <th className="px-4 py-3 text-left text-[11px] font-semibold uppercase tracking-wider text-gray-600">Account Name</th>
                  <th className="px-4 py-3 text-right text-[11px] font-semibold uppercase tracking-wider text-gray-600">Debit (₹)</th>
                  <th className="px-4 py-3 text-right text-[11px] font-semibold uppercase tracking-wider text-gray-600">Credit (₹)</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {trialBalanceData.map((a) => (
                  <tr key={a.code} className="hover:bg-royal-50/30 transition-colors">
                    ...
                    <td className="px-4 py-2.5 font-medium text-royal-950">{a.name}</td>
        ...
        {selected && (
          <div className="space-y-4 text-sm">
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-royal-50/60 rounded-lg p-3"><p className="text-[10px] uppercase tracking-wider text-gray-500 font-semibold">Code</p><p className="font-bold text-royal-950">{selected.code}</p></div>
              <div className="bg-royal-50/60 rounded-lg p-3"><p className="text-[10px] uppercase tracking-wider text-gray-500 font-semibold">Type</p><Badge tone={typeTone[selected.type] || 'gray'}> {selected.type}</Badge></div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div><p className="text-[10px] uppercase tracking-wider text-gray-500 font-semibold">Sub-Type</p><p className="font-medium">{selected.subType}</p></div>
              <div><p className="text-[10px] uppercase tracking-wider text-gray-500 font-semibold">Status</p><Badge tone={selected.isActive ? 'green' : 'gray'}>{selected.isActive ? 'Active' : 'Inactive'}</Badge></div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-royal-50/60 rounded-lg p-3"><p className="text-[10px] uppercase tracking-wider text-gray-500 font-semibold">Opening Balance</p><p className="font-bold text-gray-600">{formatINR(selected.openingBalance)}</p></div>
              <div className="bg-royal-50/60 rounded-lg p-3"><p className="text-[10px] uppercase tracking-wider text-gray-500 font-semibold">Current Balance</p><p className="font-bold text-royal-950 text-xl">{formatINR(selected.currentBalance)}</p></div>
            </div>
            <div><p className="text-[10px] uppercase tracking-wider text-gray-500 font-semibold">Account Name</p><p className="font-medium text-lg mt-1">{selected.name}</p></div>
          </div>
        )}
      </Modal>
    </div>
