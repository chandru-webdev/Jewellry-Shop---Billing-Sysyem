...
import { RefreshCw, Users, Download, Search, CheckCircle2, Loader2 } from 'lucide-react'
...
export default function CustomersSync() {
  ...
  const handlePull = () => {
    setLoading('pull')
    setTimeout(() => {
      setCustomers(customers.map((c) => c.status !== 'Synced'
        ? { ...c, status: 'Synced', syncedAt: now(), erpId: c.erpId || `CUST-${String(Math.floor(Math.random() * 900) + 100)}` }
        : c))
      setLoading(null)
      showToast('Pulled all customers from Shopify')
    }, 1500)
  }
  ...
  const handleRetry = (shopifyId) => {
    ...
    setTimeout(() => {
      setCustomers(customers.map((c) => c.shopifyId === shopifyId
        ? { ...c, status: 'Synced', syncedAt: now(), erpId: c.erpId || `CUST-${String(Math.floor(Math.random() * 900) + 100)}` }
        : c))
      setLoading(null)
      showToast('Customer synced')
    }, 1200)
  }
  ...
  const handleImport = (shopifyId) => {
    ...
    setTimeout(() => {
      setCustomers(customers.map((c) => c.shopifyId === shopifyId
        ? { ...c, status: 'Synced', syncedAt: now(), erpId: `CUST-${String(Math.floor(Math.random() * 900) + 100)}` }
        : c))
      setLoading(null)
      showToast('Customer imported')
    }, 1200)
  }

  const filtered = customers.filter((c) => {
    const q = search.toLowerCase()
    return c.name.toLowerCase().includes(q) || c.email.toLowerCase().includes(q) || c.phone.includes(q) || c.shopifyId.toLowerCase().includes(q)
  })

  const synced = customers.filter((c) => c.status === 'Synced').length
    ...
    <div>
      <PageHeader title="Customers Sync" subtitle="Sync Shopify customers with ERP" actions={
        <div className="flex gap-2">
          <Button variant="primary" size="sm" onClick={handlePull} disabled={!!loading}>
            {loading === 'pull' ? <><Loader2 size={14} className="animate-spin" /> Pulling...</> : <><Download size={14} /> Pull from Shopify</>}
          </Button>
      ...
      <div className="flex flex-wrap items-center gap-3 mb-4">
        <div className="relative flex-1 min-w-[220px] max-w-sm">
          ...
          <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search by name, email, phone or Shopify ID..." className="w-full pl-9 pr-3 py-2 text-sm rounded-lg border border-gray-200 dark:border-white/[0.08] bg-white dark:bg-[#1a1025] focus:outline-none focus:ring-2 focus:ring-royal-200 focus:border-royal-300" />
        </div>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 dark:bg-white/5 text-left">
                ...
                <th className="px-4 py-3 font-medium text-gray-600 dark:text-gray-400 text-right">Orders</th>
                <th className="px-4 py-3 font-medium text-gray-600 dark:text-gray-400 text-right">Total Spent</th>
                <th className="px-4 py-3 font-medium text-gray-600 dark:text-gray-400">ERP ID</th>
                <th className="px-4 py-3 font-medium text-gray-600 dark:text-gray-400">Status</th>
                <th className="px-4 py-3 font-medium text-gray-600 dark:text-gray-400">Last Synced</th>
            ...
            <tbody>
              {filtered.map((c, i) => (
                <tr key={c.shopifyId} className={`border-t border-gray-100 dark:border-white/[0.05] ${i % 2 === 0 ? 'bg-white dark:bg-[#1a1025]' : 'bg-gray-50/50'}`}>
                  ...
                  <td className="px-4 py-2.5 font-mono text-gray-600 dark:text-gray-400">{c.phone}</td>
                  <td className="px-4 py-2.5 text-right font-mono text-gray-600 dark:text-gray-400">{c.orders}</td>
                  ...
                  <td className="px-4 py-2.5 text-xs text-gray-500 dark:text-gray-400">{c.syncedAt || '\u2014'}</td>
                  <td className="px-4 py-2.5 text-right">
                    {c.status === 'Failed' && (
                      <Button variant="ghost" size="sm" onClick={() => handleRetry(c.shopifyId)} disabled={!!loading}>
                        {loading === `retry-${c.shopifyId}` ? <Loader2 size={12} className="animate-spin" /> : <><RefreshCw size={12} /> Retry</>}
                      </Button>
                    )}
                    {c.status === 'Pending' && (
                      <Button variant="ghost" size="sm" onClick={() => handleImport(c.shopifyId)} disabled={!!loading}>
                        {loading === `import-${c.shopifyId}` ? <Loader2 size={12} className="animate-spin" /> : <><Download size={12} /> Import</>}
                      </Button>
                    )}
                  </td>
                </tr>
