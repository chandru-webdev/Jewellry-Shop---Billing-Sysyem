  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap items-center gap-3 mb-4">
        ...
        <select value={filterModule} onChange={(e) => setFilterModule(e.target.value)} className="text-sm bg-white border border-gray-200 rounded-lg px-3 py-2 text-gray-700 focus:outline-none focus:ring-2 focus:ring-royal-500">
          {moduleOptions.map((m) => <option key={m} value={m === 'All' ? '' : m}>{m === 'All' ? 'All Modules' : m}</option>)}
        </select>
        <select value={filterAction} onChange={(e) => setFilterAction(e.target.value)} className="text-sm bg-white border border-gray-200 rounded-lg px-3 py-2 text-gray-700 focus:outline-none focus:ring-2 focus:ring-royal-500">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-royal-50/80 border-b border-gray-200">
                ...
                <th className="px-4 py-3 text-left text-[11px] font-semibold uppercase tracking-wider text-gray-600">User</th>
                <th className="px-4 py-3 text-left text-[11px] font-semibold uppercase tracking-wider text-gray-600">Action</th>
                <th className="px-4 py-3 text-center text-[11px] font-semibold uppercase tracking-wider text-gray-600">Module</th>
                <th className="px-4 py-3 text-left text-[11px] font-semibold uppercase tracking-wider text-gray-600">Entity</th>
                <th className="px-4 py-3 text-left text-[11px] font-semibold uppercase tracking-wider text-gray-600">Changes</th>
                ...
                <tr key={log.id} className="hover:bg-royal-50/30 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="font-medium text-royal-950 text-xs">{log.user}</span>
                    </div>
                  </td>
                  <td className="px-4 py-3 font-medium text-royal-950 text-xs">{log.action}</td>
                  <td className="px-4 py-3 text-center"><Badge tone="purple">{log.module}</Badge></td>
                  <td className="px-4 py-3 font-mono text-[11px] text-gray-600">{log.entity}</td>
                  <td className="px-4 py-3 text-xs text-gray-600 max-w-64 truncate">{log.changes}</td>
